 ...
 * @return {Promise<!{ mutation_hash: string }>} The result of the operation.
 */
ApiClient.prototype.getSubAccounts = function (account) {
    return httpinvoke(
        this.endpoint + "query/subaccounts?key=" + encodeURIComponent(account.toString()),
        "GET")
    .then(function (data) {
        var result = JSON.parse(data.body);
...
ApiClient.prototype.getTransaction = function (mutationHash) {
    if (typeof key === "string") {
        key = ByteBuffer.fromHex(mutationHash);
    }
